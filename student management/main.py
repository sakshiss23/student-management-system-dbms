...
def marks():
    ...
    if request.method == "POST":
        ...
        print(marks, rollno)
        marks = Marks(rollno=rollno, marks=marks)
...
def search():
    if request.method == "POST":
        ...
        marks = Marks.query.filter_by(rollno=rollno).first()
        return render_template('search.html', bio=bio, attend=attend, marks=marks)
